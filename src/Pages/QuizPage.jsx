...
// Custom components from the original code
const Card = ({ children, className, onClick }) => (
  <div onClick={onClick} className={`card ${onClick ? "clickable" : ""} ${className}`}>
    {children}
  </div>
);
...
const DyslexiaFriendlyText = ({ children, className }) => <p className={`dyslexia-text ${className}`}>{children}</p>;
...
const QuizApp = () => {
  ...
  const LandingPage = () => {
    const features = [
      {
        ...
        description: "A friendly quiz to test your knowledge.",
        view: "quiz",
      },
      {
        icon: <Lightbulb />,
        title: "Check Your Score",
        description: "See your past quiz scores.",
        view: "score_history",
      },
      {
        icon: <LineChart />,
        title: "Progress Tracking",
        description: "Visualize your journey with charts.",
        view: "progress",
      },
      {
        icon: <BookOpen />,
        title: "Resource Library",
        description: "Articles, tools, and support for dyslexia.",
        view: "resources",
      },
    ];
    ...
    //         <h2 className="text-3xl sm:text-4xl font-bold text-gray-800 tracking-wide">QUIZZZZ 🚀</h2>
    ...
    //             <CardHeader>
    //               <div className="p-4 bg-[#C6D870] rounded-full inline-flex shadow-lg">{feature.icon}</div>
    ...
    return (
      <div className="landing">
        ...
        <div className="features">
          {features.map((f) => (
            <Card key={f.title} className="feature-card" onClick={() => setCurrentView(f.view)}>
              <CardHeader>
                <div className="icon-circle">{f.icon}</div>
                <CardTitle>{f.title}</CardTitle>
              </CardHeader>
              <CardContent>
                <DyslexiaFriendlyText>{f.description}</DyslexiaFriendlyText>
              </CardContent>
            </Card>
          ))}
        </div>
  ...
  const CircularProgressBar = ({ progress, size = 80, strokeWidth = 8, color = "#B87C4C" }) => {
    ...
    return (
      <svg width={size} height={size} className="transform -rotate-90">
        ...
        />
        <text x="50%" y="50%" textAnchor="middle" stroke="none" fill="#333" fontSize="24" dy=".3em" className="transform rotate-90">
          {Math.round(progress)}%
        </text>
      </svg>
    );
  ...
  //         {/* Data points */}
  //         {data.map((d, i) => (
  //           <circle key={i} cx={xScale(i)} cy={yScale(d.score)} r="6" fill="#2563EB" stroke="#fff" strokeWidth="2" />
  //         ))}
  //       </g>
  //     </svg>
  //   );
  ...
  //       description: "Learn about what dyslexia is and how it affects reading and writing.",
  //       link: "https://opendyslexic.org/",
  //     },
  //     {
  //       title: "Phonics Exercises",
  //       description: "Interactive drills to help with sound-letter correspondence.",
  //       link: "https://www.starfall.com/h/phonic/",
  //     },
  //     {
  //       title: "Support for Parents",
  //       description: "Guidance and resources for supporting a child with dyslexia.",
  //       link: "https://www.dyslexia.com/for-parents/",
  //     },
  //     {
  //       title: "Assistive Technology",
  //       description: "Tools and apps that can help make reading and writing easier.",
  //       link: "https://www.understood.org/articles/en/assistive-technology-for-kids-with-dyslexia",
  //     },
  //   ];
  ...
  //                     className="inline-block px-4 py-2 bg-[#FFC7A7] text-[#B87C4C] font-bold rounded-full text-sm transition-transform transform hover:scale-105 hover:bg-[#C6D870] hover:text-white"
  //                   >
  //                     Visit this link box
  //                   </a>
  //                 </div>
      ...
      <div className="quiz-container">
        ...
        <div className="quiz-card">
          ...
          <div className="answers">
            {questions[currentQuestionIndex].answerOptions.map((opt, i) => (
              <button key={i} onClick={() => handleAnswerOptionClick(opt.isCorrect)} className="answer-btn">
                {opt.answerText}
              </button>
            ))}
          </div>
  ...
  const QuizResultView = () => (
    <div className="result-container">
      <div className="result-card">
        ...
        <button onClick={() => setCurrentView("score_history")}>Check All Scores</button>
        <button
          onClick={() => {
            restartQuiz();
            setCurrentView("landing");
    ...
    return (
      <div className="progress-container">
        <div className="progress-card">
          <h2 className="progress-title">Your Day-to-Day Progress</h2>
          <p className="progress-subtext">This graph shows your score on the quiz over time.</p>
